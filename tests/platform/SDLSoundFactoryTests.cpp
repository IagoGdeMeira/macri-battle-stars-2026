...
class SDLSoundFactoryFixture
{
public:
    std::filesystem::path makeTempWavPath(const char* suffix) const
    {
        const auto ticks = SDL_GetTicks();
        return std::filesystem::temp_directory_path() / (std::string("mbs_sound_factory_")
...
TEST_CASE_METHOD(SDLSoundFactoryFixture, "SDLSoundFactory returns nullptr for invalid path",
    "[integration][sdl_sound_factory]")
{
    this->configureAudioDriverForCi();
    if (!this->initAudio()) return;

    SDLSoundFactory factory;
    const auto sound = factory.createSound("this/path/does/not/exist.wav");
...
TEST_CASE_METHOD(SDLSoundFactoryFixture, "SDLSoundFactory loads a valid wav file", "[integration][sdl_sound_factory]")
{
    this->configureAudioDriverForCi();
    if (!this->initAudio()) return;
    ...
    SDLSoundFactory factory;
    const auto sound = factory.createSound(wavPath.string());
