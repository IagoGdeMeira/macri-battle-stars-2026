...
TEST_CASE("StateMapper maps known state names", "[unit][state_mapper]")
{
...
TEST_CASE("StateMapper rejects unknown state names", "[unit][state_mapper]")
{ REQUIRE_THROWS_AS(StateMapper::fromString("Flying"), std::invalid_argument); }

TEST_CASE("StateIdMapper resolves base and custom states per instance", "[unit][state_mapper]")
{
