...
public:
    AABB getAABB(UpdateContext& ctx, EntityParams params) const override
    {
        ...
        const CircleColliderComponent* circle = this->getCircleComponent(comp, {params.preferred, params.fallback});
...
private:
    ...
    {
        if (comp.has<CircleColliderComponent>(params.preferred)) return &comp.get<CircleColliderComponent>(params.preferred);
