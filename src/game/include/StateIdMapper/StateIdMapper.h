...
{
public:
    StateIdMapper() : nextCustomId(static_cast<StateId::ValueType>(StateId::BaseState::CustomStart)) {}
...
private:
    StateId::ValueType nextCustomId;

    std::unordered_map<std::string, StateId> customNameToId;
    std::unordered_map<StateId::ValueType, std::string> customIdToName;
};
